...
try {
  ...
  const summaries = {};
  const profileRunId = Date.now();

  for (const target of targets) {
    const reports = [];
    for (let index = 1; index <= runCount; index += 1) {
      const reportPath = join(reportDir, `lighthouse-${target.id}-run-${index}.json`);
      const profilePath = join('.lighthouseci', `chrome-profile-${target.id}-${profileRunId}-${index}`);
      await rm(reportPath, { force: true });
      await rm(profilePath, { recursive: true, force: true }).catch(() => undefined);
      const result = await runLighthouse(target.url, reportPath, profilePath);
      let lhr;
      try {
        ...
        throw new Error(`Lighthouse ${target.id.toUpperCase()} run ${index} did not produce a readable report: ${error instanceof Error ? error.message : String(error)}`);
      }
      if (result.code !== 0 && !isWindowsCleanupOnly(result)) {
        process.stdout.write(result.stdout);
        process.stderr.write(result.stderr);
        throw new Error(`Lighthouse ${target.id.toUpperCase()} run ${index} exited with code ${result.code ?? 1}.`);
      }
      if (result.code !== 0) console.warn(`Lighthouse ${target.id.toUpperCase()} run ${index} completed; Windows profile cleanup returned EPERM.`);
      reports.push({ lhr, reportPath });
    }

    const summary = buildMedianSummary(target.url, reports.map((item) => item.lhr));
    ...
    await writeFile(join(reportDir, `lighthouse-${target.id}-summary.json`), `${JSON.stringify(summary, null, 2)}\n`, 'utf8');
    printSummary(target.id, summary);
    assertThresholds(target.id, summary);
    summaries[target.id] = summary;
  }

  await writeFile(join(reportDir, 'lighthouse-summary.json'), `${JSON.stringify({
    generatedAt: new Date().toISOString(),
    runCountPerTarget: runCount,
    aggregation: 'median',
    targets: summaries
  }, null, 2)}\n`, 'utf8');
} finally {
...
function runLighthouse(targetUrl, reportPath, profilePath) {
...
function buildMedianSummary(targetUrl, lhrs) {
  ...
  return { generatedAt: new Date().toISOString(), url: targetUrl, runCount: lhrs.length, aggregation: 'median', categories, metrics };
}

function assertThresholds(targetId, summary) {
  const failures = [];
  ...
  if (failures.length) throw new Error(`Lighthouse ${targetId.toUpperCase()} hard gate failed:\n- ${failures.join('\n- ')}`);
}

function printSummary(targetId, summary) {
  ...
  console.log(`Lighthouse ${targetId.toUpperCase()} median (${summary.runCount} runs): ${scores.join(', ')}; ${metrics.join(', ')}`);
  console.log(`Lighthouse ${targetId.toUpperCase()} reports: ${reportDir}`);
}
